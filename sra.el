;;; sra.el -*- lexical-binding: t -*-
;;; Evaluation of Simple relational algebra exprssions on finite domains
;;; Uses efficient implementation of transitive closure on a graph of symbols
...
;; set
;; domain, collection of enumerated values from that domain

;; relation
;; domain, codomain, map, store
;; map must accept value in domain and return iterator of
;; values (rather, the enumeration of the value) in the codomain
;; memoized in store using avl-tree (hence requirement for enumeration)
...
;;       | (inverse relex)
;;
;; Note argument of complement and inverse must be stratified - evaluation of the argument
...
(cl-defstruct
    (sra-set
     (:conc-name sra--set-)
     (:constructor sra--create-set))
  domain
  elts)

(cl-defstruct
    (sra-relation
     (:conc-name sra--relation-)
     (:constructor sra--create-relation))
  domain
  codomain
  map
  edges)
